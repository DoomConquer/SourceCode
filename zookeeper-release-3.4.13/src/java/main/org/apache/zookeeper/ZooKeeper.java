 ...
 * 客户端与服务端断开连接时，所有接收的events会被触发，还没传递到客户端的watch将会丢失
 *
 */
@InterfaceAudience.Public
public class ZooKeeper {

    public static final String ZOOKEEPER_CLIENT_CNXN_SOCKET = "zookeeper.clientCnxnSocket"; // ClientCnxnSocket类路径

    protected final ClientCnxn cnxn; // 客户端连接
    private static final Logger LOG;

    static {
        //Keep these two lines together to keep the initialization order explicit
        LOG = LoggerFactory.getLogger(ZooKeeper.class);
        Environment.logEnv("Client environment:", LOG); // 日志输出环境信息
    }
    ...
    private final ZKWatchManager watchManager = new ZKWatchManager(); // 监视器管理类
    ...
    private static class ZKWatchManager implements ClientWatchManager {
        ...
        public Set<Watcher> materialize(Watcher.Event.KeeperState state,
            ...
            switch (type) {
            case None: // 客户端可能断开连接（type = EventType None && state != Watcher.Event.KeeperState.SyncConnected）
            ...
            case NodeCreated:
                synchronized (dataWatches) { // 结点数据变化
            ...
            case NodeChildrenChanged:
                synchronized (childWatches) { // 子结点变化
                    addTo(childWatches.remove(clientPath), result);
                }
                break;
            case NodeDeleted:
                synchronized (dataWatches) { // 结点删除
                    addTo(dataWatches.remove(clientPath), result);
                }
    ...
    abstract class WatchRegistration {
        private Watcher watcher;   // 监视器
        private String clientPath; // 结点路径
        ...
        /**
         * Register the watcher with the set of watches on path. 注册监视器到对应的path
     ...
     *  session建立是异步的，该方法一般会在session建立完成前返回
     */
    ...
    public ZooKeeper(String connectString, int sessionTimeout, Watcher watcher, boolean canBeReadOnly)
        ...
        LOG.info("Initiating client connection, connectString=" + connectString
                + " sessionTimeout=" + sessionTimeout + " watcher=" + watcher); // 连接信息

        watchManager.defaultWatcher = watcher; // 监视器

        ConnectStringParser connectStringParser = new ConnectStringParser(connectString); // 解析连接字符串
        HostProvider hostProvider = new StaticHostProvider(connectStringParser.getServerAddresses());
        cnxn = new ClientCnxn(connectStringParser.getChrootPath(),
                hostProvider, sessionTimeout, this, watchManager,
                getClientCnxnSocket(), canBeReadOnly); // 实例化客户端与服务端连接
        cnxn.start(); // 开启发送线程和事件处理线程
    }
     ...
     * @param sessionPasswd
     *            password for this session
     * @param canBeReadOnly 客户端能连接到read-only服务器，这些服务器可能是形成分区（脑裂）的服务器
     *            (added in 3.4) whether the created client is allowed to go to
     *            read-only mode in case of partitioning. Read-only mode
     *            basically means that if the client can't find any majority
     *            servers but there's partitioned server it could reach, it
    ...
    public ZooKeeper(String connectString, int sessionTimeout, Watcher watcher,
        ...
        cnxn.seenRwServerBefore = true; // since user has provided sessionId 已经提供sessionId，之前已经连接过服务器
        cnxn.start();
    }
     ...
     * re-connect. 获取当前sessionId
     *
     * This method is NOT thread safe 非线程安全
     *
     ...
     * may change after a re-connect. 获取session超时时间，重连后可能会改变
     *
     ...
     * specified during construction). 注册默认监视器（会覆盖实例化zookeeper时指定的监视器）
     *
     * @param watcher
     */
     ...
     * 关闭客户端连接，关闭后session将失效，关联的零时结点将被删除，关联的监视器将被触发
     *
     * @throws InterruptedException
     */
    public synchronized void close() throws InterruptedException {
        if (!cnxn.getState().isAlive()) { // 已经关闭
    ...
    private String prependChroot(String clientPath) {
        if (cnxn.chrootPath != null) {
            // handle clientPath = "/"
            if (clientPath.length() == 1) { // clientPath = /
     ...
     *                and/or sequential
     * @return the actual path of the created node 返回结点的路径
    ...
    public String create(final String path, byte data[], List<ACL> acl, CreateMode createMode)
        ...
        PathUtils.validatePath(clientPath, createMode.isSequential()); // 验证路径是否合法

        final String serverPath = prependChroot(clientPath); // 服务端path路径
        ...
        request.setAcl(acl);
        ReplyHeader r = cnxn.submitRequest(h, request, response, null); // 同步提交请求
        if (r.getErr() != 0) {
    ...
    /**
     * The asynchronous version of create. 异步创建结点
     *
    ...
    public void multi(Iterable<Op> ops, MultiCallback cb, Object ctx) {
        List<OpResult> results = validatePath(ops); // 验证op中path路径是否合法
        if (results.size() > 0) { // 参数检验异常
        ...
        for (Op op : ops) {
            try {
                ...
                ErrorResult err = new ErrorResult(
                        KeeperException.Code.BADARGUMENTS.intValue()); // 参数异常
                results.add(err);
                error = true;
                continue;
            ...
            ErrorResult err = new ErrorResult(
                    KeeperException.Code.RUNTIMEINCONSISTENCY.intValue()); // 不一致异常
            results.add(err);
        }
        if (false == error) {
            results.clear();
    ...
    private Op withRootPrefix(Op op) {
        if (null != op.getPath()) {
            ...
            if (!op.getPath().equals(serverPath)) { // 客户端path和服务端不相等，添加上chroot路径
    ...
    protected List<OpResult> multiInternal(MultiTransactionRecord request)
        ...
        for (OpResult result : results) { // 结果中存在ErrorResult
        ...
        if (fatalError != null) { // 存在Error，抛出KeeperException异常，让使用者处理，异常信息在KeeperException的results中
     ...
     * 封装multi，可以通过transaction().create().delete()等添加操作到一个事务中，通过commit提交（调用zk.multi()函数）
     *
     * @since 3.4.0
     *
    ...
    public Stat exists(final String path, Watcher watcher)
        throws KeeperException, InterruptedException { // 判断结点是否存在，存在返回stat，否则返回null
        final String clientPath = path;
        PathUtils.validatePath(clientPath);
        ...
        ReplyHeader r = cnxn.submitRequest(h, request, response, wcb); // 同步提交请求
        if (r.getErr() != 0) {
            if (r.getErr() == KeeperException.Code.NONODE.intValue()) { // 结点不存在，返回null
                return null;
            }
            throw KeeperException.create(KeeperException.Code.get(r.getErr()), clientPath);
        }

        return response.getStat().getCzxid() == -1 ? null : response.getStat(); // zxid为-1，结果无效（可能zxid落后leader结点的zxid）
    }
     ...
     *                the node path
     * @param watch
     *                whether need to watch this node 若为true使用默认watcher
     * @return the stat of the node of the given path; return null if no such a
     *         node exists.
     * @throws KeeperException If the server signals an error
     * @throws InterruptedException If the server transaction is interrupted.
    ...
    /**
     * The asynchronous version of exists. 异步exists
     *
    ...
    public byte[] getData(final String path, Watcher watcher, Stat stat)
        ...
        if (stat != null) {
            DataTree.copyStat(response.getStat(), stat); // 复制结点状态信息到stat
        }
        return response.getData(); // 返回结点内容数据
    }
    ...
    /**
     * The asynchronous version of getData. 异步获取结点数据
     *
     ...
     *                the data to set
     * @param version
     *                the expected matching version 如果version为-1，匹配所有的版本
    ...
    public Stat setData(final String path, byte data[], int version) throws KeeperException, InterruptedException {
        ...
        request.setPath(serverPath);
        request.setData(data);
        request.setVersion(version); // 版本，只有版本和内存数据库中的匹配时才能设置成功
        SetDataResponse response = new SetDataResponse();
        ReplyHeader r = cnxn.submitRequest(h, request, response, null);
        if (r.getErr() != 0) {
            throw KeeperException.create(KeeperException.Code.get(r.getErr()), clientPath);
    ...
    /**
     * The asynchronous version of setData. 异步设置数据
     *
    ...
    /**
     * Return the ACL and stat of the node of the given path. 获取ACL及状态stat
     * <p>
     * A KeeperException with error code KeeperException.NoNode will be thrown
     * if no node with the given path exists.
     *
    ...
    /**
     * The asynchronous version of getACL. 异步获取ACL
     *
    ...
    public Stat setACL(final String path, List<ACL> acl, int aclVersion)
        ...
        request.setAcl(acl);
        request.setVersion(aclVersion); // aclVersion需要匹配结点的acl version
    ...
    /**
     * The asynchronous version of setACL. 异步设置ACL
     *
    ...
    public void setACL(final String path, List<ACL> acl, int version, StatCallback cb, Object ctx) {
        ...
        request.setPath(serverPath);
        request.setAcl(acl);
        request.setVersion(version);// version需要匹配结点的acl version
        SetACLResponse response = new SetACLResponse();
        cnxn.queuePacket(h, new ReplyHeader(), request, response, cb, clientPath, serverPath, ctx, null);
    }
    ...
    /**
     * The asynchronous version of getChildren. 异步获取子结点列表
     *
    ...
    public List<String> getChildren(final String path, Watcher watcher, Stat stat)
        ...
        if (stat != null) {
            DataTree.copyStat(response.getStat(), stat); // 复制状态stat
        }
        return response.getChildren();
    }
    ...
    protected boolean testableWaitForShutdown(int wait) throws InterruptedException {
        cnxn.sendThread.join(wait);  // 关闭发送请求线程
        if (cnxn.sendThread.isAlive()) return false;
        cnxn.eventThread.join(wait); // 关闭处理事件线程
