 ...
 * 客户端连接（维护一个服务列表，有问题透明地转到其他可用服务上）
 */
public class ClientCnxn {
     ...
     */
    private static final int SET_WATCHES_MAX_LENGTH = 128 * 1024; // 限制SetWatches请求（128kb）超过服务配置的jute.maxBuffer大小

    /** This controls whether automatic watch resetting is enabled.
     * Clients automatically reset watches during session reconnect, this
     * option allows the client to turn off this behavior by setting
     * the environment variable "zookeeper.disableAutoWatchReset" to "true" */
    private static boolean disableAutoWatchReset; // 是否允许连接断开重连时自动重置watch
    ...
    private final CopyOnWriteArraySet<AuthData> authInfo = new CopyOnWriteArraySet<AuthData>(); // 客户端认证信息
     ...
     * value.
     */
    private volatile int negotiatedSessionTimeout; // 和服务端negotiate超时

    private int readTimeout;

    private final int sessionTimeout; // session超时

    private final ZooKeeper zooKeeper;

    private final ClientWatchManager watcher; // 客户端监视器

    private long sessionId; // 会话id

    private byte sessionPasswd[] = new byte[16]; // 会话密码
     ...
     */
    private boolean readOnly; // 客户端只读模式

    final String chrootPath; // 根path

    final SendThread sendThread;   // 发送请求线程

    final EventThread eventThread; // 处理事件线程
     ...
     */
    volatile boolean seenRwServerBefore = false; // 当连接到可读写的服务成功时才设置为true
    ...
    public String toString() {
        ...
        SocketAddress local = sendThread.getClientCnxnSocket().getLocalSocketAddress();   // 客户端地址
        SocketAddress remote = sendThread.getClientCnxnSocket().getRemoteSocketAddress(); // 服务端地址
        sb
    ...
    static class Packet {
        ...
        String clientPath; // 客户端path
        /** Servers's view of the path (may differ due to chroot) **/
        String serverPath; // 服务端path

        boolean finished; // 是否完成发送
        ...
        public void createBB() {
            try {
                ...
                if (request instanceof ConnectRequest) { // 连接请求
                ...
                this.bb.putInt(this.bb.capacity() - 4); // 写入请求数据长度
                this.bb.rewind(); // bb缓存区当前位置复位
            } catch (IOException e) {
    ...
    public ClientCnxn(String chrootPath, HostProvider hostProvider, int sessionTimeout, ZooKeeper zooKeeper,
        ...
        readTimeout = sessionTimeout * 2 / 3; // 读超时时间，会话超时的2/3
    ...
    private static class WatcherSetEventPair {
        private final Set<Watcher> watchers; // 需要被通知的监视器
    ...
    class EventThread extends ZooKeeperThread {
        private final LinkedBlockingQueue<Object> waitingEvents = new LinkedBlockingQueue<Object>(); // 等待处理事件队列
        ...
        private volatile boolean isRunning = false; // 标识线程运行状态
       ...
       public void queuePacket(Packet packet) {
          if (wasKilled) { // 线程被killed（不是kill -9）
             synchronized (waitingEvents) {
                if (isRunning) waitingEvents.add(packet); // 运行状态isRunning还在运行，添加到等待队列，虽然wasKilled线程还会继续处理
                else processEvent(packet); // 直接处理
             }
          } else {
        ...
        public void run() {
           try {
              ...
              while (true) {
                 ...
                 if (wasKilled)
                    synchronized (waitingEvents) {
                       if (waitingEvents.isEmpty()) { // 当等待队列为空时，正式停止线程
                          isRunning = false;
                          break;
                       }
                    }
       ...
       private void processEvent(Object event) {
          try {
              if (event instanceof WatcherSetEventPair) { // 触发所有监视该事件的监视器
                  // each watcher will process the event
                  WatcherSetEventPair pair = (WatcherSetEventPair) event;
                  for (Watcher watcher : pair.watchers) {
                      try {
                          watcher.process(pair.event); // 触发监视器
                      } catch (Throwable t) {
                  ...
                  if (p.cb == null) { // callback为空，没有异步回调
                  ...
                  } else if (p.response instanceof GetDataResponse) { // 获取数据回调
                  ...
                  } else if (p.response instanceof GetACLResponse) { // 获取ACL权限回调
                  ...
                  } else if (p.response instanceof GetChildrenResponse) { // 获取子结点信息回调
                  ...
                  } else if (p.response instanceof GetChildren2Response) { // 获取子结点信息回调
                  ...
                  } else if (p.response instanceof CreateResponse) { // 创建结点回调
                  ...
                  } else if (p.response instanceof MultiResponse) { // 多操作回调
    ...
    // 如果是异步请求将数据包加入等待队列，等待处理异步回调，同步请求直接处理
    private void finishPacket(Packet p) {
        if (p.watchRegistration != null) {
            p.watchRegistration.register(p.replyHeader.getErr());
        }

        if (p.cb == null) { // 不是异步回调，直接处理
            synchronized (p) {
                p.finished = true;
                p.notifyAll();
            }
    ...
    private volatile long lastZxid; // 最近处理的zxid
     ...
     * 该类处理发送请求队列和产生心跳，同时生成ReadThread
     */
    class SendThread extends ZooKeeperThread {
        private long lastPingSentNs; // 上一次ping消息发送时间（纳秒数）
        private final ClientCnxnSocket clientCnxnSocket; // 客户端socket连接
        private Random r = new Random(System.nanoTime());
        private boolean isFirstConnect = true; // 第一次连接标志

        // 读取响应数据
        void readResponse(ByteBuffer incomingBuffer) throws IOException {
            ByteBufferInputStream bbis = new ByteBufferInputStream(incomingBuffer);
            BinaryInputArchive bbia = BinaryInputArchive.getArchive(bbis);
            ReplyHeader replyHdr = new ReplyHeader(); // 响应头
            ...
            if (replyHdr.getXid() == -4) { // 认证数据包
                // -4 is the xid for AuthPacket               
                if(replyHdr.getErr() == KeeperException.Code.AUTHFAILED.intValue()) { // 认证失败
                    state = States.AUTH_FAILED;                    
                    eventThread.queueEvent(new WatchedEvent(Watcher.Event.EventType.None,
                            Watcher.Event.KeeperState.AuthFailed, null) ); // 生成认证失败事件让eventThread处理
                }
                if (LOG.isDebugEnabled()) {
            ...
            if (replyHdr.getXid() == -1) { // 通知消息
                ...
                // 转化服务端path为客户端path（客户端可能有chrootPath）
                if (chrootPath != null) {
            ...
            // 如果是正在进行SASL认证过程，直接返回响应数据包，不用加入等待队列处理
            if (clientTunneledAuthenticationInProgress()) {
                GetSASLRequest request = new GetSASLRequest();
                request.deserialize(bbia, "token");
                zooKeeperSaslClient.respondToServer(request.getToken(), ClientCnxn.this); // 响应服务端token
                return;
            }

            Packet packet; // 等待响应的数据包
             ...
             */
            try {
                if (packet.requestHeader.getXid() != replyHdr.getXid()) { // 响应的数据包和等待响应的数据包不相符，抛出异常
                ...
                if (replyHdr.getZxid() > 0) {
                    lastZxid = replyHdr.getZxid(); // 记录最近zxid
                }
                ...
                finishPacket(packet); // 如果是异步请求，将数据包加入等待队列，等待eventThread处理异步回调
            }
        }

        SendThread(ClientCnxnSocket clientCnxnSocket) {
            super(makeThreadName("-SendThread()"));
            state = States.CONNECTING; // 正在连接
            this.clientCnxnSocket = clientCnxnSocket;
            setDaemon(true); // 设置为守护线程
        }

        // TODO: can not name this method getState since Thread.getState() already exists
        // It would be cleaner to make class SendThread an implementation of
        // Runnable
        /**
         * 获取zk连接状态，这里不能用函数getState，和Thread.getState()冲突
        ...
        void primeConnection() throws IOException {
            ...
            ConnectRequest conReq = new ConnectRequest(0, lastZxid, sessionTimeout, sessId, sessionPasswd); // 连接请求
            synchronized (outgoingQueue) {
                // We add backwards since we are pushing into the front
                // Only send if there's a pending watch
                // TODO: here we have the only remaining use of zooKeeper in this class. It's to be eliminated!
                if (!disableAutoWatchReset) { // 客户端刚开始连接服务端时，如果客户端有监视器，发送这些监视器到服务端
                    ...
                    if (!dataWatches.isEmpty()
                        ...
                        while (dataWatchesIter.hasNext()
                            ...
                            while (batchLength < SET_WATCHES_MAX_LENGTH) { // 是否超过长度限制
                                final String watch;
                            ...
                            outgoingQueue.addFirst(packet); // 添加到发送队列头
                        }
                    }
                }

                for (AuthData id : authInfo) { // 发送认证认证请求
            ...
            clientCnxnSocket.enableReadWriteOnly(); // 允许clientCnxnSocket可读写
            if (LOG.isDebugEnabled()) {
        ...
        private void sendPing() {
            lastPingSentNs = System.nanoTime(); // 记录上一次发送ping时间（纳秒）
        ...
        private void startConnect(InetSocketAddress addr) throws IOException {
            ...
            if (ZooKeeperSaslClient.isEnabled()) {
                ...
                } catch (LoginException e) { // sasl登录失败（new ZooKeeperSaslClient的时候会进行登录认证）
        ...
        @Override
        public void run() {
            clientCnxnSocket.introduce(this, sessionId); // 设置属性
            ...
            while (state.isAlive()) {
                try {
                    if (!clientCnxnSocket.isConnected()) { // 未连接
                        ...
                        startConnect(serverAddress); // 开始连接服务端（进行sasl登录，连接socket）
                        clientCnxnSocket.updateLastSendAndHeard();
                    }

                    if (state.isConnected()) { // 已经连接上服务端
                        // determine whether we need to send an AuthFailed event.
                        if (zooKeeperSaslClient != null) {
                            boolean sendAuthEvent = false;
                            if (zooKeeperSaslClient.getSaslState() == ZooKeeperSaslClient.SaslState.INITIAL) { // sasl还处于INITIAL状态
                                try {
                                    zooKeeperSaslClient.initialize(ClientCnxn.this); // 进行初始化
                            ...
                            if (authState != null) {
                                if (authState == KeeperState.AuthFailed) {
                                    ...
                                    if (authState == KeeperState.SaslAuthenticated) { // 认证通过
                                        sendAuthEvent = true;
                                    }
                                }
                            }

                            if (sendAuthEvent == true) { // 生成事件
                    ...
                    if (state.isConnected()) {
                        ...
                        int timeToNextPing = readTimeout / 2 - clientCnxnSocket.getIdleSend() - 
                        		((clientCnxnSocket.getIdleSend() > 1000) ? 1000 : 0); // 下一次发送ping请求的时间
                    ...
                    if (closing) { // 关闭连接
                        ...
                        if (e instanceof SessionExpiredException) { // session过期
                            LOG.info(e.getMessage() + ", closing socket connection");
                        } else if (e instanceof SessionTimeoutException) { // session超时
                            LOG.info(e.getMessage() + RETRY_CONN_MSG);
                        } else if (e instanceof EndOfStreamException) { // 流异常结束
                            LOG.info(e.getMessage() + RETRY_CONN_MSG);
                        ...
                        cleanup(); // 清理客户端连接
                        if (state.isAlive()) { // 如果state状态还是alive，断开连接
        ...
        private void pingRwServer() throws RWServerFoundException, UnknownHostException {
            ...
            try {
                ...
                sock.getOutputStream().write("isro".getBytes()); // 是否只读
                sock.getOutputStream().flush(); // 向客户端发送isro
                sock.shutdownOutput();
                br = new BufferedReader(
                        new InputStreamReader(sock.getInputStream()));
                result = br.readLine(); // 获取返回结果
                ...
                LOG.warn("Exception while seeking for r/w server " +
                        e.getMessage(), e);
            } finally { // 关闭socket，BufferedReader资源
                if (sock != null) {
                    try {
                        sock.close();
                    } catch (IOException e) {
        ...
        // 清理客户端连接（待发送队列和等待响应队列）
        private void cleanup() {
            clientCnxnSocket.cleanup(); // 关闭socket
            synchronized (pendingQueue) {
                for (Packet p : pendingQueue) { // 发送连接断开请求
                    conLossPacket(p);
                }
                pendingQueue.clear();
            }
        ...
        void onConnected(int _negotiatedSessionTimeout, long _sessionId,
            ...
            if (negotiatedSessionTimeout <= 0) { // negotiatedSessionTimeout时间小于等于0，关闭连接
                state = States.CLOSED;
            ...
            if (!readOnly && isRO) { // 读写客户端连接到只读服务上
            ...
            state = (isRO) ?
                    States.CONNECTEDREADONLY : States.CONNECTED; // 连接到zk服务的状态
            seenRwServerBefore |= !isRO; // 可读写服务端
     ...
     * 关闭连接，包括发送关闭session给服务端，关闭send/event线程
     *
     * @throws IOException
     */
    ...
    public ReplyHeader submitRequest(RequestHeader h, Record request,
        ...
        synchronized (packet) {
            while (!packet.finished) { // 等待数据包完成（读取服务端响应时完成数据包处理）
                packet.wait();
            }
        }
        return r;
    ...
    Packet queuePacket(RequestHeader h, ReplyHeader r, Record request,
        ...
        synchronized (outgoingQueue) {
            ...
            if (!state.isAlive() || closing) { // 连接丢失或关闭
                conLossPacket(packet);
            } else {
                // If the client is asking to close the session then
                // mark as closing
                if (h.getType() == OpCode.closeSession) { // 关闭session
                    closing = true;
                }
                outgoingQueue.add(packet);
            }
        }
        sendThread.getClientCnxnSocket().wakeupCnxn(); // 唤醒cnxn
        return packet;
    }
