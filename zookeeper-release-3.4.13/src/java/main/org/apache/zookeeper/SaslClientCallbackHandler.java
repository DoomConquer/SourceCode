...
public class SaslClientCallbackHandler implements CallbackHandler {
    ...
    private final String entity; // 客户端或集群中learner
        ...
        for (Callback callback : callbacks) {
            if (callback instanceof NameCallback) {
                ...
                if (callback instanceof PasswordCallback) {
                    ...
                    if (password != null) { // 设置password
                        pc.setPassword(this.password.toCharArray());
                    } else { // 没有密码（通过keytab方式不用交互输入密码，先将密码保存到keytab文件，kerberos自动验证）
                    ...
                    if (callback instanceof RealmCallback) {
                        ...
                        if (callback instanceof AuthorizeCallback) { // 处理认证回调
