...
public class ZKDatabase {
    ...
    protected ConcurrentHashMap<Long, Integer> sessionsWithTimeouts; // session及其过期时间
    protected FileTxnSnapLog snapLog; // 快照和事务日志处理类
    protected long minCommittedLog, maxCommittedLog; // 事务请求最小、最大zxid
    public static final int commitLogCount = 500;
    protected static int commitLogBuffer = 700;
    protected LinkedList<Proposal> committedLog = new LinkedList<Proposal>(); // 事务请求（可能会改变结点状态）队列
    protected ReentrantReadWriteLock logLock = new ReentrantReadWriteLock(); // 事务日志读写锁
    volatile private boolean initialized = false; // 初始化标志
     ...
     * filetxnsnaplog和zkdatabase是一一对应关系
     *
     * @param snapLog the FileTxnSnapLog mapping this zkdatabase
     */
    public ZKDatabase(FileTxnSnapLog snapLog) {
        dataTree = new DataTree(); // 初始化DataTree
    ...
    public synchronized LinkedList<Proposal> getCommittedLog() {
        ...
        // 如果该线程没有持有logLock锁，返回committedLog复制
     ...
     * 根据数据快照文件和事务日志文件恢复数据库
     *
     * @return the last valid zxid on disk
     * @throws IOException
     */
    public long loadDataBase() throws IOException {
        long zxid = snapLog.restore(dataTree, sessionsWithTimeouts, commitProposalPlaybackListener); // 恢复数据库
        initialized = true;
        return zxid;
    }
    ...
    public long fastForwardDataBase() throws IOException {
        long zxid = snapLog.fastForwardFromEdits(dataTree, sessionsWithTimeouts, commitProposalPlaybackListener); // 快速恢复
        initialized = true;
        return zxid;
    }

    // 添加事务请求（可能会改变结点状态的请求）到committedLog队列
     ...
     * 维护最近的committedLog列表，用于follower快速同步
     *
     * @param request committed request
     */
    public void addCommittedProposal(Request request) {
        WriteLock wl = logLock.writeLock(); // 获取写锁
        try {
            wl.lock();
            if (committedLog.size() > commitLogCount) { // 如果committedLog列表size超过commitLogCount（维护最近500个），移除头部元素
                committedLog.removeFirst();
                minCommittedLog = committedLog.getFirst().packet.getZxid();
            }
            if (committedLog.size() == 0) { // 记录最小和最大committedLog
                minCommittedLog = request.zxid;
                maxCommittedLog = request.zxid;
            }

            byte[] data = SerializeUtils.serializeRequest(request); // 序列化请求为字节数组
            QuorumPacket pp = new QuorumPacket(Leader.PROPOSAL, request.zxid, data, null);
            Proposal p = new Proposal(); // 生成提案
            p.packet = pp;
            p.request = request;
            committedLog.add(p);
    ...
    /**
     * the node count of the datatree 数据库中结点数量
    ...
    /**
     * the process txn on the data 数据库上处理请求事务
     *
    ...
    /**
     * get the datanode for this path 获取结点
     *
    ...
    /**
     * get data and stat for a path 获取结点数据
     *
    ...
    /**
     * set watches on the datatree 设置监视器
     *
    ...
    /**
     * get acl for a path 获取path对应的ACL权限
     *
    ...
    /**
     * get children list for this path 获取子结点列表
     *
    ...
    /**
     * check if the path is special or not 检查是否是zookeeper系统结点路径
     *
    ...
    /**
     * get the acl size of the datatree 获取数据库上ACL数量
     *
    ...
    /**
     * deserialize a snapshot from an input archive 反序列化快照数据
     *
    ...
    /**
     * serialize the snapshot 序列化快照数据到OutputArchive
     *
    ...
    /**
     * roll the underlying log 日志重新写到一个文件
     */
    ...
    /**
     * commit to the underlying transaction log 提交潜在的事务日志
     *
     * @throws IOException
     */
    ...
    /**
     * close this database. free the resources 关闭数据库，日志文件，释放资源
     * @throws IOException
     */
