 ...
 * logs.
 * 该类用于根据设置的autopurge.purgeInterval自动清除数据快照和事务日志，最多保留最近autopurge.snapRetainCount数量数据快照和事务日志
 */
public class DatadirCleanupManager {
    ...
    private PurgeTaskStatus purgeTaskStatus = PurgeTaskStatus.NOT_STARTED; // 未开始

    private final String snapDir;    // 数据快照路径

    private final String dataLogDir; // 事务日志路径

    private final int snapRetainCount; // 保留日志数量

    private final int purgeInterval; // 任务执行间隔
     ...
     */
    public void start() { // 开始定时任务
        if (PurgeTaskStatus.STARTED == purgeTaskStatus) { // 已经开始
            LOG.warn("Purge task is already running.");
            return;
        }
        // Don't schedule the purge task with zero or negative purge interval.
        if (purgeInterval <= 0) { // purgeInterval不能小于或等于0
        ...
        timer.scheduleAtFixedRate(task, 0, TimeUnit.HOURS.toMillis(purgeInterval)); // 定时执行

        purgeTaskStatus = PurgeTaskStatus.STARTED;
    }

    /**
     * Shutdown the purge task. 关闭定时任务
     */
    ...
    static class PurgeTask extends TimerTask {
        ...
        public void run() {
            ...
            try {
                PurgeTxnLog.purge(new File(logsDir), new File(snapsDir), snapRetainCount); // 清理文件
            } catch (Exception e) {
