...
public class Vote {
    ...
    final private int version; // 选票版本
    
    final private long id;   // 被选举的服务id
    
    final private long zxid; // 被选举的事务id
    
    final private long electionEpoch; // 逻辑时钟，用来判断多个投票是否在同一轮周期中，该值在服务端是一个自增序列，每进入新的一轮投票进行加一
    ...
    public boolean equals(Object o) {
         ...
         */
        if ((state == ServerState.LOOKING) || (other.state == ServerState.LOOKING)) { // 处于选举阶段
            return (id == other.id
                    && zxid == other.zxid
                    && electionEpoch == other.electionEpoch
                    && peerEpoch == other.peerEpoch);
        } else {
            if ((version > 0x0) ^ (other.version > 0x0)) { // 一个为0x0，一个不是，只比较服务id是否相等
                return id == other.id;
            } else {
