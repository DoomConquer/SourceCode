...
public class KerberosName {
   ...
   */
  private static List<Rule> rules; // 名称规则

  private static String defaultRealm; // 默认域空间（类似namespace）
  
  static {
    try {
      defaultRealm = KerberosUtil.getDefaultRealm(); // 获取默认realm名称
    } catch (Exception ke) {
   ...
   * kerberos名称解析规则，规则包含三个部分，分别是：base，filter和substitution，例如
   * 规则RULE:[2:\$1@\$0](.*@\\QONE.COM\\E$)s/@\\QONE.COM\\E$，[]中的是base，()中的是filter，后面的是替换substitution
   * 详情可参看https://docs.hortonworks.com/HDPDocuments/HDP2/HDP-2.4.0/bk_Security_Guide/content/create_mappings_betw_principals_and_unix_usernames.html
   */
  ...
  public static void setConfiguration() throws IOException {
    //获取规则 如：Dzookeeper.security.auth_to_local=RULE:[2:\$1@\$0](.*@\\QONE.COM\\E$)s/@\\QONE.COM\\E$//DEFAULT
