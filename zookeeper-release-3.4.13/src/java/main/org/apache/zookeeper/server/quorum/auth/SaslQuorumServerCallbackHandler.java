...
public class SaslQuorumServerCallbackHandler implements CallbackHandler {
    ...
    public SaslQuorumServerCallbackHandler(Configuration configuration,
        ...
        if (configurationEntries == null) { // 找不到配置文件中serverSection对应的域
        ...
        for(AppConfigurationEntry entry : configurationEntries) {
            ...
            for(Map.Entry<String, ?> pair : options.entrySet()) { // 从配置文件中获取用户信息（user_开头）
    ...
    private void handleAuthorizeCallback(AuthorizeCallback ac) {
        String authenticationID = ac.getAuthenticationID(); // 验证id
        String authorizationID = ac.getAuthorizationID();   // 授权id

        boolean authzFlag = false;
        // 1. Matches authenticationID and authorizationID
        authzFlag = authenticationID.equals(authorizationID); // 首先比较两个id是否相等
        ...
        if (authzFlag) { // 检查连接的host是否在授权的host中
