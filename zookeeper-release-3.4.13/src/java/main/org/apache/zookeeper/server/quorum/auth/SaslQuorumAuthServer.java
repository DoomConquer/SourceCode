...
public class SaslQuorumAuthServer implements QuorumAuthServer {
    ...
    private final static int MAX_RETRIES = 5; // 重新尝试次数
    private final Login serverLogin;
    private final boolean quorumRequireSasl; // 是否需要sasl认证
    ...
    public SaslQuorumAuthServer(boolean quorumRequireSasl, String loginContext, Set<String> authzHosts)
        ...
        try {
            ...
            if (entries == null || entries.length == 0) { // 找不到loginContext（serverSection域）对应的jaas配置
            ...
            throw new SaslException(
                    "Failed to initialize authentication mechanism using SASL", e);
        }
    }
    ...
    public void authenticate(Socket sock, DataInputStream din)
        ...
        SaslServer ss = null;
        try {
            if (!QuorumAuth.nextPacketIsAuth(din)) { // 数据包不需要认证
            ...
            byte[] token = receive(din); // 获取token
            int tries = 0; // 重试次数
            ...
            while (!ss.isComplete()) {
                ...
                if (!ss.isComplete()) {
                    ...
                    if (++tries > MAX_RETRIES) { // 超过重试次数MAX_RETRIES
            ...
            if (ss.isComplete()) { // 认证完成
                send(dout, challenge, QuorumAuth.Status.SUCCESS); // 发送成功消息给learner
                LOG.info("Successfully completed the authentication using SASL. learner addr: {}",
                        sock.getRemoteSocketAddress());
            }
        } catch (Exception e) {
            try {
                if (dout != null) { // 发送错误信息给learner
