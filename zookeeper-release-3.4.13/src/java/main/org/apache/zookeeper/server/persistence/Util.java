...
public class Util {
     ...
     * using one of the {@see makeLogName} or {@see makeSnapshotName}.
     * 从快照文件或日志文件中提取zxid
     * 
    ...
    public static long getZxidFromName(String name, String prefix) {
        ...
        if (nameParts.length == 2 && nameParts[0].equals(prefix)) {
            try {
                zxid = Long.parseLong(nameParts[1], 16); // 以16进制将zxid转换成long
    ...
    public static boolean isValidSnapshot(File f) throws IOException {
        if (f == null || Util.getZxidFromName(f.getName(), FileSnap.SNAPSHOT_FILE_PREFIX) == -1) // 无效快照文件
            return false;
    ...
    public static void writeTxnBytes(OutputArchive oa, byte[] bytes)
        ...
        oa.writeByte((byte) 0x42, "EOR"); // 以'B'结尾
    }
