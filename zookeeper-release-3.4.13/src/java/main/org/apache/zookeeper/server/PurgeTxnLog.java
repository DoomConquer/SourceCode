...
public class PurgeTxnLog {
    ...
    private static final String PREFIX_SNAPSHOT = "snapshot"; // 快照文件前缀
    private static final String PREFIX_LOG = "log";           // 日志文件前缀
    ...
    public static void purge(File dataDir, File snapDir, int num) throws IOException {
        ...
        List<File> snaps = txnLog.findNRecentSnapshots(num); // 最近num个快照文件
        int numSnaps = snaps.size();
        if (numSnaps > 0) {
            purgeOlderSnapshots(txnLog, snaps.get(numSnaps - 1)); // 删除之前的快照文件及日志文件
        }
    }
    ...
    static void purgeOlderSnapshots(FileTxnSnapLog txnLog, File snapShot) {
        final long leastZxidToBeRetain = Util.getZxidFromName(
                snapShot.getName(), PREFIX_SNAPSHOT); // 应该保留的最近zxid
         ...
         */
        final Set<File> retainedTxnLogs = new HashSet<File>(); // 应该保留的文件
        retainedTxnLogs.addAll(Arrays.asList(txnLog.getSnapshotLogs(leastZxidToBeRetain)));

        /**
         * Finds all candidates for deletion, which are files with a zxid in their name that is less
         * than leastZxidToBeRetain.  There's an exception to this rule, as noted above.
         * 文件过滤器（过滤出需要删除的文件，及zxid小于leastZxidToBeRetain的文件）
         */
        ...
        List<File> files = new ArrayList<File>(); // 所有需要删除的文件
        File[] fileArray = txnLog.getDataDir().listFiles(new MyFileFilter(PREFIX_LOG)); // 过滤事务日志文件
        if (fileArray != null) {
            files.addAll(Arrays.asList(fileArray));
        }

        // add all non-excluded snapshot files to the deletion list
        fileArray = txnLog.getSnapDir().listFiles(new MyFileFilter(PREFIX_SNAPSHOT));   // 过滤数据快照文件
        if (fileArray != null) {
            files.addAll(Arrays.asList(fileArray));
        }

        // remove the old files
        for(File f: files) { // 删除文件
    ...
    public static void main(String[] args) throws IOException {
        ...
        purge(dataDir, snapDir, num); // 清理文件
    }
