...
public class VerGen {
    ...
    public static void generateFile(File outputDir, Version version, String rev, String buildDate) throws IOException
    {
        ...
        File pkgdir = new File(outputDir, path); // 包路径，outputDir为父路径
        if (!pkgdir.exists()) { // 不存在目录
            // create the pkg directory
            boolean ret = pkgdir.mkdirs(); // 创建目录
            if (!ret) {
                System.out.println("Cannnot create directory: " + path);
                System.exit(1);
            }
        } else if (!pkgdir.isDirectory()) { // 该路径不是目录
            // not a directory
            System.out.println(path + " is not a directory.");
            System.exit(1);
        }
        File file = new File(pkgdir, TYPE_NAME + ".java"); // 类名Info.java
        FileWriter w = null;
        try {
    ...
    public static void main(String[] args) {
        if (args.length != 3) // 3个参数
            printUsage();
        try {
            Version version = parseVersionString(args[0]); // 解析第一个参数获取Version
            if (version == null) {
                System.err.println(
                        "Invalid version number format, must be \"x.y.z(-.*)?\"");
                System.exit(1);
            }
            String rev = args[1]; // git version number
            if (rev == null || rev.trim().isEmpty()) {
                rev = "-1";
            } else {
                rev = rev.trim();
            }
            generateFile(new File("."), version, rev, args[2]); // new File(".")表示当前目录
