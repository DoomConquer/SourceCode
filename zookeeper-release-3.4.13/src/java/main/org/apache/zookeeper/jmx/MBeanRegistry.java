...
public class MBeanRegistry {
    ...
    private static volatile MBeanRegistry instance = new MBeanRegistry(); // 注册类
    ...
    /**
     * Registers a new MBean with the platform MBean server. 注册MBean
     * @param bean the bean being registered
     * @param parent if not null, the new bean will be registered as a child
     * node of this parent. 如果patent不为空，bean将作为parent的子结点被注册
     */
    public void register(ZKMBeanInfo bean, ZKMBeanInfo parent)
        ...
        String path = null;
        if (parent != null) {
            path = mapBean2Path.get(parent); // 父结点路径
            assert path != null;
        }
        path = makeFullPath(path, parent);
        if(bean.isHidden()) // bean不可见
            return;
        ObjectName oname = makeObjectName(path, bean); // 生成ObjectName
        try {
    ...
    /**
     * Unregister the MBean identified by the path. 注销path对应的MBean
     * @param path
     * @param bean
     */
    ...
    /**
     * Unregister all currently registered MBeans 注销所有MBean
     */
    ...
    /**
     * Generate a filesystem-like path. 生成类文件系统路径
     ...
     * 将路径/a/b/c转换为name0=a,name1=b,name2=c形式
     */
    ...
    protected ObjectName makeObjectName(String path, ZKMBeanInfo bean)
        ...
        beanName.deleteCharAt(beanName.length() - 1); // 删除最后的逗号
        try {
