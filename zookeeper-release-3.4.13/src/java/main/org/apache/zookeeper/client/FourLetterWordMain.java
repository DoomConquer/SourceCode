...
public class FourLetterWordMain {
    ...
    private static final int DEFAULT_SOCKET_TIMEOUT = 5000; // 默认socket超时
    ...
    public static String send4LetterWord(String host, int port, String cmd, int timeout)
        ...
        try {
            sock.setSoTimeout(timeout); // 设置超时
            sock.connect(hostaddress, timeout); // 连接服务器
            OutputStream outstream = sock.getOutputStream();
            outstream.write(cmd.getBytes()); // 发送四字命令
            outstream.flush();
