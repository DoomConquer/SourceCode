...
public class ZooKeeperSaslClient {
    public static final String LOGIN_CONTEXT_NAME_KEY = "zookeeper.sasl.clientconfig"; // 客户端配置文件中sasl配置的名称
    public static final String ENABLE_CLIENT_SASL_KEY = "zookeeper.sasl.client"; // 客户端是否允许sasl
    public static final String ENABLE_CLIENT_SASL_DEFAULT = "true"; // 默认允许
     ...
     */
    public static boolean isEnabled() { // 是否允许sasl
    ...
    private static Login login = null; // 客户端登录，所有与服务器相关的连接共享login
    private SaslClient saslClient; // sasl客户端
    private boolean isSASLConfigured = true; // sasl是否完成配置（默认完成，失败时设置为false）
    ...
    private final String configStatus; // 保存配置时消息

    public SaslState getSaslState() {
        return saslState;
    }

    public String getLoginContext() { // 获取LoginContext名称
        if (login != null)
    ...
    public ZooKeeperSaslClient(final String serverPrincipal)
        ...
        AppConfigurationEntry entries[] = null; // 客户端配置属性
        ...
        if (entries != null) {
            ...
            this.saslClient = createSaslClient(serverPrincipal, clientSection); // 创建SaslClient
        } else { // 客户端没使用sasl（输出原因）
            ...
            if (explicitClientSection != null) { // 没找到jaas配置中对应section
            ...
            if (System.getProperty(Environment.JAAS_CONF_KEY)  != null) { // 设置了jaas配置，但配置有问题
    ...
    // 是否完成（Determines whether the authentication exchange has completed）
    ...
    public static class ServerSaslResponseCallback implements AsyncCallback.DataCallback {
        public void processResult(int rc, String path, Object ctx, byte data[], Stat stat) {
            ...
            client.respondToServer(usedata, (ClientCnxn)ctx); // 响应服务器token信息（data）
        }
    }
    ...
    private SaslClient createSaslClient(final String servicePrincipal,
        ...
        try {
            if (!initializedLogin) { // 没有初始化
                synchronized (ZooKeeperSaslClient.class) { // 所有服务端相关的连接共享login
                    if (login == null) {
                        ...
                        login.startThreadIfNeeded(); // 启动周期性refresh线程
    ...
    public void respondToServer(byte[] serverToken, ClientCnxn cnxn) {
        ...
        if (!(saslClient.isComplete())) { // authentication exchange没有完成
            try {
                saslToken = createSaslToken(serverToken);
                if (saslToken != null) {
                    sendSaslPacket(saslToken, cnxn); // 发送sasl token
                }
        ...
        if (saslClient.isComplete()) { // authentication exchange完成
            // GSSAPI: server sends a final packet after authentication succeeds or fails.
    ...
    private void sendSaslPacket(byte[] saslToken, ClientCnxn cnxn)
      throws SaslException{
        ...
        try {
            cnxn.sendPacket(request, response, cb, ZooDefs.OpCode.sasl); // 发送sasl请求
        } catch (IOException e) {
    ...
    public void initialize(ClientCnxn cnxn) throws SaslException {
        ...
        if (saslState == SaslState.INITIAL) { // 未初始化
