...
// 语法分析器，该包下的所有文件是JavaCC根据rcc.jj文件生成的，该类是编译自动生成record类主类。
// 该类可以读取zookeeper.jute文件，根据词法解析这个文件，然后根据语法生成对应的需要序列化的
// record类。整个生成过程是配置在build.xml文件中，编译的时候被执行。
public class Rcc implements RccConstants {
  ...
  final public JFile Include() throws ParseException {
    String fname;
    Token t;
    jj_consume_token(INCLUDE_TKN); // 消费include标记（c语言中）
