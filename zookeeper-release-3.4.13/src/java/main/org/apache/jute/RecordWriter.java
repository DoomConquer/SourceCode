...
public class RecordWriter {
    ...
    static private OutputArchive getCsvArchive(OutputStream out) throws IOException {
        try {
    ...
    static private OutputArchive getXmlArchive(OutputStream out) throws IOException {
        return new XmlOutputArchive(out);
    }

    // 构造不同类型的OutputArchive
    static HashMap constructFactory() {
        HashMap factory = new HashMap();
        Class[] params = { OutputStream.class };
        try {
            factory.put("binary",
                    BinaryOutputArchive.class.getDeclaredMethod("getArchive", params));
            factory.put("csv",
                    CsvOutputArchive.class.getDeclaredMethod("getArchive", params));
            factory.put("xml",
                    XmlOutputArchive.class.getDeclaredMethod("getArchive", params));
    ...
    // 根据不同类型创建OutputArchive
    static private OutputArchive createArchive(OutputStream out, String format) throws IOException {
    ...
    /**
     * Serialize a record 序列化一个record
