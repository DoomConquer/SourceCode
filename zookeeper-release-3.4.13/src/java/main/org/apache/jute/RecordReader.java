...
public class RecordReader {
    ...
    // 初始化不同类型的InputArchive方法
    static {
        archiveFactory = new HashMap();
        Class[] params = { InputStream.class };
        try {
            archiveFactory.put("binary",
                    BinaryInputArchive.class.getDeclaredMethod("getArchive", params));
            archiveFactory.put("csv",
                    CsvInputArchive.class.getDeclaredMethod("getArchive", params));
            archiveFactory.put("xml",
                    XmlInputArchive.class.getDeclaredMethod("getArchive", params));
    ...
    /**
     * Deserialize a record 反序列化一个record
