...
class LocalCache<K, V> extends AbstractMap<K, V> implements ConcurrentMap<K, V> {
  ...
  enum Strength {
    ...
    // 根据value引用类型创建相应的ValueReference
  ...
  /** Creates new entries. */
  // 创建新的entry工厂，根据不同的引用类型创建不同的entry
  enum EntryFactory {
    STRONG {
      @Override
  ...
  void processPendingNotifications() {
    ...
    while ((notification = removalNotificationQueue.poll()) != null) {
      try {
        removalListener.onRemoval(notification); // 处理回调
      } catch (Throwable e) {
  ...
  static class Segment<K, V> extends ReentrantLock {
    ...
    // segment table（ReferenceEntry引用数组）
      ...
      try {
        ...
        for (e = first; e != null; e = e.getNext()) {
          ...
          if (e.getHash() == hash
            ...
            if (valueReference.isLoading()) { // value已经存在
    ...
    V compute(K key, int hash, BiFunction<? super K, ? super V, ? extends V> function) {
      ...
      try {
        ...
        AtomicReferenceArray<ReferenceEntry<K, V>> table = this.table;
        int index = hash & (table.length() - 1);
        ReferenceEntry<K, V> first = table.get(index); // table中第一个元素
        ...
        for (e = first; e != null; e = e.getNext()) {
          ...
          if (e.getHash() == hash
              ...
              && map.keyEquivalence.equivalent(key, entryKey)) {
            valueReference = e.getValueReference();
            if (map.isExpired(e, now)) { // 如果超时了，将key，value添加到通知队列等待缓存失效回调处理
              // This is a duplicate check, as preWriteCleanup already purged expired
              // entries, but let's accomodate an incorrect expiration queue.
              enqueueNotification(
                  entryKey,
     ...
     * is already loading.
     * 插入LoadingValueReference类型value
     */
    @Nullable
      ...
      try {
        ...
        for (e = first; e != null; e = e.getNext()) {
          ...
          if (e.getHash() == hash
            ...
            e.setValueReference(loadingValueReference); // 设置value
        ...
        e = newEntry(key, hash, first);
        e.setValueReference(loadingValueReference);
        table.set(index, e); // 设置value对应的entry
        return loadingValueReference;
      } finally {
        unlock();
        postWriteCleanup();
    ...
    void drainKeyReferenceQueue() {
      ...
      while ((ref = keyReferenceQueue.poll()) != null) {
        ...
        if (++i == DRAIN_MAX) { // 不是每次都清空引用队列中的所有元素
          break;
        }
      }
    }

    // 移除value引用队列中被垃圾回收的元素
    @GuardedBy("this")
    ...
    boolean containsKey(Object key, int hash) {
      try {
        if (count != 0) { // read-volatile
          ...
          return e.getValueReference().get() != null; // key对应的value不存在（value的引用类型为weak或soft，已经被垃圾回收）
        }
    ...
    void expand() {
      ...
      if (oldCapacity >= MAXIMUM_CAPACITY) { // 超过最大限制
        return;
      }
      ...
      int newCount = count;
      AtomicReferenceArray<ReferenceEntry<K, V>> newTable = newEntryArray(oldCapacity << 1); // 扩大两倍
      threshold = newTable.length() * 3 / 4; // 门限0.75
      ...
      try {
        ...
        for (ReferenceEntry<K, V> e = first; e != null; e = e.getNext()) {
          ...
          if (e.getHash() == hash
            ...
            ValueReference<K, V> valueReference = e.getValueReference();
            V entryValue = valueReference.get();
            if (entryValue == null) { // 可能被垃圾回收了，此时删除key，返回null
              if (valueReference.isActive()) {
                // If the value disappeared, this entry is partially collected.
                int newCount = this.count - 1;
                ++modCount;
    ...
    // 移除链表上的entry结点（如果entry前有被垃圾回收的结点也一同被移除）
    @GuardedBy("this")
    @Nullable
    ReferenceEntry<K, V> removeEntryFromChain(
      ...
      for (ReferenceEntry<K, V> e = first; e != entry; e = e.getNext()) {
        ReferenceEntry<K, V> next = copyEntry(e, newFirst); // 新创建一个entry，e的next结点为newFirst
        if (next != null) {
          newFirst = next; // 整个过程将entry前的结点翻转了一遍，最后将entry后续的结点连接上，这样删除了entry
        } else { // 移除被垃圾回收掉的entry
          removeCollectedEntry(e);
          newCount--;
        }
      ...
      try {
        ...
        for (ReferenceEntry<K, V> e = first; e != null; e = e.getNext()) {
          ...
          if (e.getHash() == hash
            ...
            if (v == valueReference) { // 找到回应的valueReference
              ++modCount;
              ReferenceEntry<K, V> newFirst =
                  removeValueFromChain(
                      first,
        ...
        if (e == entry) {
          ...
          ReferenceEntry<K, V> newFirst =
              removeValueFromChain(
                  ...
                  cause);
          newCount = this.count - 1;
          table.set(index, newFirst); // 设置table index的第一个结点
          this.count = newCount; // write-volatile
          return true;
        }
      }
  ...
  public @Nullable V get(@Nullable Object key) {
    ...
    int hash = hash(key); // 求key的hash值
    return segmentFor(hash).get(key, hash); // 从对应的segment中找到key对应的value
  }
  ...
  ImmutableMap<K, V> getAll(Iterable<? extends K> keys) throws ExecutionException {
    ...
    Map<K, V> result = Maps.newLinkedHashMap();
    Set<K> keysToLoad = Sets.newLinkedHashSet(); // 缓存中不存在的key
    for (K key : keys) {
      V value = get(key);
    ...
    try {
      ...
      if (!keysToLoad.isEmpty()) {
        try {
          Map<K, V> newEntries = loadAll(keysToLoad, defaultLoader); // 加载keys
  ...
  abstract class HashIterator<T> implements Iterator<T> {
    ...
    boolean advanceTo(ReferenceEntry<K, V> entry) {
      try {
        long now = ticker.read();
        K key = entry.getKey();
        V value = getLiveValue(entry, now); // 获取key对应的value
        if (value != null) {
  ...
  abstract class AbstractCacheSet<T> extends AbstractSet<T> {
    @Weak final ConcurrentMap<?, ?> map; // 并发map
   ...
   * LocalManualCache.
   * 序列化Cache（LocalManualCache）的代理类
   *
   ...
   * 序列化LoadingCache（LocalLoadingCache）的代理类
   *
   * <p>Unfortunately, readResolve() doesn't get called when a circular dependency is present, so
   * the proxy must be able to behave as the cache itself.
   */
