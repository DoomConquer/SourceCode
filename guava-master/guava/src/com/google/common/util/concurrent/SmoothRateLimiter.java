...
abstract class SmoothRateLimiter extends RateLimiter {
   ...
   * 这里从thresholdPermits到0的时间取warmupPeriod / 2（It's an arbitrary value）见：
   * https://groups.google.com/forum/#!topic/guava-discuss/w2f8HvnE47Q
   *
   ...
   *       </blockquote>
   * </ul>
   * 该限流器是平滑预热到给定的速率，刚开始已经积累了maxPermits数量许可，第一个请求进来可以获取到许可（没超过最大值），下一个请求进来时计算需要等待的时间，
   * 这个时间包含上一个请求的生成许可时间（这些许可已经在桶中，在storedPermits）
   */
  static final class SmoothWarmingUp extends SmoothRateLimiter {
     ...
     */
    private double slope; // 直线斜率

    private double thresholdPermits; // 从固定速率warming up的门限值
    private double coldFactor; // 冷却因子，当时间达到coldFactor * stableInterval时达到最大许可数（需要开始冷却，即等待消耗许可）
    ...
    // 设置thresholdPermits和storedPermits
    @Override
    void doSetRate(double permitsPerSecond, double stableIntervalMicros) {
      double oldMaxPermits = maxPermits;
      double coldIntervalMicros = stableIntervalMicros * coldFactor;
      thresholdPermits = 0.5 * warmupPeriodMicros / stableIntervalMicros; // 按照coldFactor取3，门限是maxPermits的一半
      maxPermits =
          thresholdPermits + 2.0 * warmupPeriodMicros / (stableIntervalMicros + coldIntervalMicros);
      slope = (coldIntervalMicros - stableIntervalMicros) / (maxPermits - thresholdPermits); // 斜率
    ...
    long storedPermitsToWaitTime(double storedPermits, double permitsToTake) {
      double availablePermitsAboveThreshold = storedPermits - thresholdPermits; // 可用的超过门限的许可
      long micros = 0;
    ...
    // 根据计算warmupPeriodMicros / maxPermits和stableIntervalMicros相等
    @Override
   ...
   * 突发到给定速率的限流器，假如现在许可数量很多，同时很多请求进来就能同时拿到许可形成很高并发流量
   */
  static final class SmoothBursty extends SmoothRateLimiter {
    ...
    // 设置maxPermits和storedPermits
    @Override
    void doSetRate(double permitsPerSecond, double stableIntervalMicros) {
      double oldMaxPermits = this.maxPermits;
      maxPermits = maxBurstSeconds * permitsPerSecond; // 计算最大许可数
      if (oldMaxPermits == Double.POSITIVE_INFINITY) { // Double.POSITIVE_INFINITY为正无穷大Infinity
        // if we don't special-case this, we would get storedPermits == NaN, below
        storedPermits = maxPermits;
      } else {
        storedPermits =
            (oldMaxPermits == 0.0) // 初始状态
                ? 0.0 // initial state
                : storedPermits * maxPermits / oldMaxPermits; // 根据速率调节storedPermits的大小
      }
    }
  ...
  final long reserveEarliestAvailable(int requiredPermits, long nowMicros) {
    ...
    double storedPermitsToSpend = min(requiredPermits, this.storedPermits); // 积累的许可需要消耗的个数
    double freshPermits = requiredPermits - storedPermitsToSpend; // 需要获取的许可
    long waitMicros =
        storedPermitsToWaitTime(this.storedPermits, storedPermitsToSpend)
            + (long) (freshPermits * stableIntervalMicros);

    // 更新下一次请求获取许可时间和积累的许可数
   ...
   * 获取从[(storedPermits - permitsToTake), storedPermits]个许可需要的时间，满足0 <= permitsToTake <= storedPermits
   * 对于固定速率的情况，该函数永远返回0
   *
  ...
  void resync(long nowMicros) {
    ...
    if (nowMicros > nextFreeTicketMicros) {
      // 新的请求和记录的下一次许可时间之间生成的新的许可
      double newPermits = (nowMicros - nextFreeTicketMicros) / coolDownIntervalMicros();
      storedPermits = min(maxPermits, storedPermits + newPermits); // 更新积累的许可，不超过maxPermits
      nextFreeTicketMicros = nowMicros; // 更新下一次许可的时间
    }
  }
}
