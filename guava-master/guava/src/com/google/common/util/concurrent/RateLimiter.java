...
import org.checkerframework.checker.nullness.qual.MonotonicNonNull;import java.util.concurrent.TimeUnit;
...
public abstract class RateLimiter {
   ...
   * 创建一个rate为指定的permitsPerSecond的限流器
   *
   ...
   * 创建一个rate慢慢增加至指定permitsPerSecond的限流器
   *
  ...
  // 单例生成mutexDoNotUseDirectly（double check）
  ...
  public double acquire(int permits) {
    ...
    stopwatch.sleepMicrosUninterruptibly(microsToWait); // 阻塞等待（不会被中断）
  ...
  public boolean tryAcquire(int permits, long timeout, TimeUnit unit) {
    ...
    synchronized (mutex()) {
      ...
      if (!canAcquire(nowMicros, timeoutMicros)) { // 超时时间内不能获取
        return false;
      } else {
        microsToWait = reserveAndGetWaitLength(permits, nowMicros);
      }
    }
    stopwatch.sleepMicrosUninterruptibly(microsToWait); // 等待获取许可
    return true;
  }

  // 是否能在timeoutMicros超时前获取许可
    ...
    public static SleepingStopwatch createFromSystemTimer() {
      return new SleepingStopwatch() {
        ...
        protected void sleepMicrosUninterruptibly(long micros) {
          if (micros > 0) {
            Uninterruptibles.sleepUninterruptibly(micros, MICROSECONDS); // 不中断休眠
          }
        }
      };
    }
