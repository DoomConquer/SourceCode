 ...
 */
@Beta
public final class BloomFilter<T> implements Predicate<T>, Serializable { // 支持序列化
  /**
   * A strategy to translate T instances, to {@code numHashFunctions} bit indexes.
   * 将T转换成numHashFunctions位索引
   *
   ...
   * object可能在布隆过滤器中（不知一定在，如果返回false，一定不在）
   */
   ...
   * Creates a {@link BloomFilter} with the expected number of insertions and expected false
   * positive probability.
   * 创建布隆过滤器，expectedInsertions为预期存入数据量
   *
   * <p>Note that overflowing a {@code BloomFilter} with significantly more elements than specified,
   * will result in its saturation, and a sharp deterioration of its false positive probability.
   *
  ...
  static <T> BloomFilter<T> create(
     ...
     */
    long numBits = optimalNumOfBits(expectedInsertions, fpp); // 需要总bit位数
    int numHashFunctions = optimalNumOfHashFunctions(expectedInsertions, numBits); // 需要的hash函数个数
    try {
   ...
   * 计算需要多少位才能保证fpp达到要求，公式：m = -n * lnp / ln2^2
   *
  ...
  */
  ...
  private static class SerialForm<T> implements Serializable {
    ...
    SerialForm(BloomFilter<T> bf) {
      this.data = LockFreeBitArray.toPlainArray(bf.bits.data); // 变成字节数组
