...
public class Timeout {
   ...
   */
  private boolean hasDeadline;   // 是否有deadline（如果deadlineNanoTime定义了为true）
  private long deadlineNanoTime; // deadline纳秒数
  private long timeoutNanos;     // timeout纳秒数
  ...
  /** Set a deadline of now plus {@code duration} time. （deadline = 现在 + duration）*/
   ...
   * 超过截止时间或线程被中断时抛出InterruptedIOException异常
   */
  ...
  public final void waitUntilNotified(Object monitor) throws InterruptedIOException {
    try {
      ...
      } else if (hasDeadline) { // timeoutNanos = 0
      ...
      if (waitNanos > 0L) {
        ...
        // 等待waitMillis毫秒（waitNanos / 1000000L有余数，等待waitMillis + 1毫秒）
        monitor.wait(waitMillis, (int) (waitNanos - waitMillis * 1000000L));
        elapsedNanos = System.nanoTime() - start;
      }

      // Throw if the timeout elapsed before the monitor was notified.
      if (elapsedNanos >= waitNanos) { // 超时
