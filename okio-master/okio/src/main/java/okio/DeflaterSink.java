...
public final class DeflaterSink implements Sink {
  ...
  private final Deflater deflater; // 压缩
  ...
  private void deflate(boolean syncFlush) throws IOException {
    ...
    while (true) {
      Segment s = buffer.writableSegment(1); // 获取可写的segment
      ...
      if (deflated > 0) {
        ...
        sink.emitCompleteSegments(); // 将buffer中的数据写到sink中
      } else if (deflater.needsInput()) { // 数据为空
        if (s.pos == s.limit) { // 分配了一个tail segment，但是没有使用，回收segment
