...
final class SegmentedByteString extends ByteString {
  final transient byte[][] segments; // 存放每个segment数据
  final transient int[] directory;   // 记录每个segment中数据的有效长度（递增）和开始位置
  ...
  SegmentedByteString(Buffer buffer, int byteCount) {
    ...
    for (Segment s = buffer.head; offset < byteCount; s = s.next) {
      ...
      offset += s.limit - s.pos;
      if (offset > byteCount) { // 有共享数据
        offset = byteCount; // Despite sharing more bytes, only report having up to byteCount.
      }
      directory[segmentCount] = offset; // 记录有效数据长度
      directory[segmentCount + segments.length] = s.pos; // 记录每个segment中数据开始位置
      s.shared = true;
      segmentCount++;
    }
  }
  ...
  @Override void write(Buffer buffer) {
    ...
    buffer.size += segmentOffset; // buffer长度增加
  }

  // 判断offset开始的数据和other从Offset开始的数据是否相等（比较byteCount长度）
  @Override public boolean rangeEquals(int offset, ByteString other, int otherOffset, int byteCount) {
  ...
  // 判断offset开始的数据和other字节数组从Offset开始的数据是否相等（比较byteCount长度）
