...
public final class InflaterSource implements Source {
  ...
  private final Inflater inflater; // 解压
  ...
  @Override public long read(Buffer sink, long byteCount) throws IOException {
    ...
    while (true) {
      boolean sourceExhausted = refill(); // source数据填充inflater

      // Decompress the inflater's compressed data into the sink.
      try {
        Segment tail = sink.writableSegment(1);
        int toRead = (int) Math.min(byteCount, Segment.SIZE - tail.limit);
        int bytesInflated = inflater.inflate(tail.data, tail.limit, toRead); // inflater解压数据到sink中
        if (bytesInflated > 0) {
        ...
        if (inflater.finished() || inflater.needsDictionary()) {
          releaseInflatedBytes(); // 移除source中处理完的数据
          if (tail.pos == tail.limit) { // 回收没有使用的segment
        ...
        if (sourceExhausted) throw new EOFException("source exhausted prematurely"); // source过早被消耗完
   ...
   * was exhausted. 填充source数据到inflater
   */
  public boolean refill() throws IOException {
    if (!inflater.needsInput()) return false;

    releaseInflatedBytes(); // 移除处理完的数据
    ...
    inflater.setInput(head.data, head.pos, bufferBytesHeldByInflater); // inflater中输入source数据
    return false;
  }
