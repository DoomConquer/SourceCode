...
public final class Buffer implements BufferedSource, BufferedSink, Cloneable {
  ...
  @Nullable Segment head; // segment头
  long size; // buffer的字节长度
  ...
  public Buffer writeTo(OutputStream out, long byteCount) throws IOException {
    ...
    while (byteCount > 0) {
      ...
      if (s.pos == s.limit) { // 这个segment中数据写完了，回收segment
        Segment toRecycle = s;
   ...
   * without harming throughput. 可刷新的字节数
   */
  public long completeSegmentByteCount() {
    ...
    if (tail.limit < Segment.SIZE && tail.owner) { // 如果最后一个segment可写要排除
  ...
  @Override public byte readByte() {
    ...
    if (pos == limit) { // 该segment数据读完了，回收segment
      head = segment.pop();
  ...
  @Override public short readShort() {
    ...
    if (pos == limit) { // segment读完了，回收segment
      head = segment.pop();
      SegmentPool.recycle(segment);
    } else {
      segment.pos = pos;
  ...
  // 从offset开始读取byteCount字节数据待sink字节数组中
  @Override public int read(byte[] sink, int offset, int byteCount) {
   ...
   * with a buffer will return its segments to the pool. 丢弃该buffer中所有的数据
   */
  public void clear() {
    try {
      skip(size);
  ...
  /** Discards {@code byteCount} bytes from the head of this buffer. 跳过byteCount字节数据 */
  @Override public void skip(long byteCount) throws EOFException {
    while (byteCount > 0) {
      ...
      if (head.pos == head.limit) { // 回收segment
  ...
  // 将source中数据（offset开始，byteCount个字节）写入buffer
  @Override public Buffer write(byte[] source, int offset, int byteCount) {
    ...
    while (offset < limit) {
      Segment tail = writableSegment(1); // 获取要写入的segment

      int toCopy = Math.min(limit - offset, Segment.SIZE - tail.limit);
      System.arraycopy(source, offset, tail.data, tail.limit, toCopy); // 数据写入tail（从limit位置开始写）
   ...
   * 返回一个尾部可写至少minimumCapacity字节的segment
   */
  Segment writableSegment(int minimumCapacity) {
    if (minimumCapacity < 1 || minimumCapacity > Segment.SIZE) throw new IllegalArgumentException();

    if (head == null) { // segment为空
  ...
  // 从source的buffer中数据写到该buffer中，只需要改变指针，不会浪费cpu和内存资源
  @Override public void write(Buffer source, long byteCount) {
    ...
    while (byteCount > 0) {
      ...
      if (byteCount < (source.head.limit - source.head.pos)) { // source的head segment数据超过byteCount，直接拷贝
      ...
      if (head == null) { // 当前buffer为空（还没有head segment）
        head = segmentToMove;
        head.next = head.prev = head;
      } else { // 将source的head segment插入当前buffer的尾部
        Segment tail = head.prev;
  ...
  // 从当前buffer中读取byteCount字节数据写到sink中
  ...
  // 字节b在buffer中从fromIndex到toIndex的位置，-1表示不存在
  @Override public long indexOf(byte b, long fromIndex, long toIndex) {
    if (fromIndex < 0 || toIndex < fromIndex) {
      throw new IllegalArgumentException(String.format("size=%s fromIndex=%s toIndex=%s", size, fromIndex, toIndex));
    }
    ...
    findSegmentAndOffset: {
      ...
      if (s == null) { // segment为空
        // No segments to scan!
        return -1L;
      } else if (size - fromIndex < fromIndex) { // 从尾部开始扫描
    ...
    while (offset < toIndex) { // 扫描segment
      byte[] data = s.data;
