 ...
 * size. When a producer thread outruns（超过） its consumer the buffer fills up and eventually writes to
 ...
 * sink -> source管道，生产者将数据写入sink，消费者从source开始消费
 */
public final class Pipe {
  final long maxBufferSize; // buffer最大缓冲数据长度
  final Buffer buffer = new Buffer(); // buffer缓冲
  boolean sinkClosed;
    ...
    @Override public void write(Buffer source, long byteCount) throws IOException {
      synchronized (buffer) {
        ...
        while (byteCount > 0) {
          ...
          if (bufferSpaceAvailable == 0) { // 没有数据等待
          ...
          buffer.write(source, bytesToWrite); // 将source缓冲中数据写到buffer中
  ...
  final class PipeSource implements Source {
    ...
    @Override public long read(Buffer sink, long byteCount) throws IOException {
      synchronized (buffer) {
        ...
        long result = buffer.read(sink, byteCount); // 从buffer中读数据写到sink中
