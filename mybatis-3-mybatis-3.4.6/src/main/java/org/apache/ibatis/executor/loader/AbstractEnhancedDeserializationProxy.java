...
public abstract class AbstractEnhancedDeserializationProxy {
  ...
  private final Map<String, ResultLoaderMap.LoadPair> unloadedProperties; // 保存未执行延迟加载的LoadPair
  private final ObjectFactory objectFactory;
  private final List<Class<?>> constructorArgTypes;
  private final List<Object> constructorArgs;
  private final Object reloadingPropertyLock; // 加载锁，防止多线程同时加载
  private boolean reloadingProperty;          // 是否已经加载属性（已经执行延迟加载）
  ...
  public final Object invoke(Object enhanced, Method method, Object[] args) throws Throwable {
    ...
    try {
      if (WRITE_REPLACE_METHOD.equals(methodName)) { // writeReplace方法
        ...
        synchronized (this.reloadingPropertyLock) {
          if (!FINALIZE_METHOD.equals(methodName) && PropertyNamer.isProperty(methodName) && !reloadingProperty) {
            final String property = PropertyNamer.methodToProperty(methodName); // 根据方法名获取属性
            ...
            if (unloadedProperties.containsKey(propertyKey)) {
              ...
              if (loadPair != null) {
                try {
                  ...
                  reloadingProperty = false; // 可能有多个属性需要延迟加载，加载完将该标志复位（虽然已经有reloadingPropertyLock，防止重入锁）
                }
              } else {
