...
public class ResultLoaderMap {
  ...
  private final Map<String, LoadPair> loaderMap = new HashMap<String, LoadPair>(); // 存储延迟加载的ResultLoader，key为属性名称，value为封装的ResultLoader

  public void addLoader(String property, MetaObject metaResultObject, ResultLoader resultLoader) {
    String upperFirst = getUppercaseFirstProperty(property);
    // 同一个resultMap中存在相同的lazy加载属性
  ...
  public static class LoadPair implements Serializable {
     ...
     */
    private final transient Object serializationCheck = new Object(); // 标识是否是被序列化，serializationCheck==null标识被序列化
    /**
    ...
    public void load(final Object userObject) throws SQLException {
      ...
      // 使用一个新的Executor，因为executors不是线程安全的
      if (this.serializationCheck == null) {
        final ResultLoader old = this.resultLoader;
        this.resultLoader = new ResultLoader(old.configuration, new ClosedExecutor(), old.mappedStatement,
                old.parameterObject, old.targetType, old.cacheKey, old.boundSql);
      }
      // 通过resultLoader的loadResult方法执行sql查询加载结果
      this.metaResultObject.setValue(property, this.resultLoader.loadResult());
    }

    // 通过configurationFactory反射获取Configuration
  ...
  // 关闭的Executor（用于传参占位），每次加载时需要生成Executor
