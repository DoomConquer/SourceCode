...
public class ScriptRunner {
  ...
  private boolean stopOnError; // error时停止
  private boolean throwWarning;
  private boolean autoCommit;
  private boolean sendFullScript; // 执行整个脚本，即一次性获取整个脚本Reader
  private boolean removeCRs; // 是否移除回车换行
  private boolean escapeProcessing = true; // 是否允许进行转义处理
  ...
  private void rollbackConnection() {
    try {
      if (!connection.getAutoCommit()) { // 如果autoCommit为false，会回滚执行的SQL
        connection.rollback();
      }
    } catch (Throwable t) {
      // ignore
  ...
  private void handleLine(StringBuilder command, String line) throws SQLException {
    ...
    if (lineIsComment(trimmedLine)) { // 注释行
      ...
      if (matcher.find()) {
        delimiter = matcher.group(5); // 匹配第五个()里的
      }
      println(trimmedLine);
      ...
      command.setLength(0);
    } else if (trimmedLine.length() > 0) { // 没遇到分隔符，再读下一行直到有分隔符形成完整的一行
      command.append(line);
      command.append(LINE_SEPARATOR);
    }
  }
