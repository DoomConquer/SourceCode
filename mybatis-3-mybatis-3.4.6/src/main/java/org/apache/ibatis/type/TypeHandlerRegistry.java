 ...
 * TypeHandler注册中心，该类保存了所有类型的TypeHandler
 */
public final class TypeHandlerRegistry {
  ...
  private final Map<Class<?>, TypeHandler<?>> ALL_TYPE_HANDLERS_MAP = new HashMap<Class<?>, TypeHandler<?>>(); // key为TypeHandler的类类型
  ...
  // 根据type和jdbcType获取TypeHandler，如果没有返回null对应的TypeHandler
  ...
  private <T> TypeHandler<T> getTypeHandler(Type type, JdbcType jdbcType) {
    ...
    if (jdbcHandlerMap != null) {
      ...
      if (handler == null) {
        // #591
        // 找jdbcHandlerMap中一个独占的handler，如果不止一个返回null
  ...
  public void register(Class<?> typeHandlerClass) {
    boolean mappedTypeFound = false;
    MappedTypes mappedTypes = typeHandlerClass.getAnnotation(MappedTypes.class); // 根据类上MappedTypes注解
    if (mappedTypes != null) {
  ...
  // 扫描packageName包下所有非匿名类、接口、抽象类的TypeHandler类
